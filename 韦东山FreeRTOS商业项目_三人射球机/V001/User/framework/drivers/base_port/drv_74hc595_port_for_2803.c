...
/*--------------------------------------------------------------------------------*/
static void SCK_H(void){GPIO_SetBits(GPIOB, GPIO_Pin_4);}
static void SCK_L(void){GPIO_ResetBits(GPIOB, GPIO_Pin_4);}

static void RCK_H(void){GPIO_SetBits(GPIOB, GPIO_Pin_3);}
static void RCK_L(void){GPIO_ResetBits(GPIOB, GPIO_Pin_3);}

static void SDO_H(void){GPIO_SetBits(GPIOA, GPIO_Pin_15);}
static void SDO_L(void){GPIO_ResetBits(GPIOA, GPIO_Pin_15);}
...
hc595_pt hc595_for_2803;					//对外接口
void init_drv_hc595_for_2803(void)
{
	int i=0;
	static u8 flag = 0;
	if(flag == 1)return ;
	...
	for(i=0;i<ChipNum;i++) OUT[i] = 0;
 	write_bus_data();		
	update_data_hc595();					
	/*接口外放*/
	hc595_for_2803 = hc595;
